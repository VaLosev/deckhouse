...
func (l *Loader) restoreAbsentModulesFromReleases(ctx context.Context) error {
	...
	for _, release := range releases.Items {
		...
		if err != nil {
			l.log.Errorf("failed to get module pull override for the '%s' module: %v", moduleName, err)
			return err
		}
		...
		if err = l.client.Get(ctx, types.NamespacedName{Name: moduleSource}, source); err != nil {
			return fmt.Errorf("failed to get the '%s' module source for the '%s' module: %v", source.Name, moduleName, err)
		}
...
func (l *Loader) deleteModulesWithAbsentRelease(ctx context.Context) error {
	...
	fsModulesLinks, err := l.readModulesFromFS(symlinksDir)
	if err != nil {
	...
	l.log.Debugf("%d releases found", len(releases.Items))
...
func (l *Loader) readModulesFromFS(dir string) (map[string]string, error) {
...
func (l *Loader) restoreAbsentModulesFromOverrides(ctx context.Context) error {
	...
	if len(currentNodeName) == 0 {
		return fmt.Errorf("determine the node name deckhouse pod is running on: missing or empty DECKHOUSE_NODE_NAME env")
	}
	...
	for _, mpo := range mpos.Items {
		...
		moduleName := mpo.Name
		moduleSource := mpo.Spec.Source
		moduleImageTag := mpo.Spec.ImageTag
		moduleWeight := mpo.Status.Weight

		// get relevant module source
		source := new(v1alpha1.ModuleSource)
		if err := l.client.Get(ctx, client.ObjectKey{Name: moduleSource}, source); err != nil {
			return fmt.Errorf("get the '%s' module source for the '%s' module: %w", moduleSource, moduleName, err)
		}
		...
		if moduleWeight == 0 {
			...
			if err != nil {
				return fmt.Errorf("get the '%s' module definition from repository: %w", moduleName, err)
			}
		...
		if annotationNodeName, set := mpo.GetAnnotations()[v1alpha1.ModuleReleaseAnnotationDeckhouseNodeName]; !set || annotationNodeName != currentNodeName {
			l.log.Infof("reinitialize the '%s' module pull override due to stale/absent %s annotation", moduleName, v1alpha1.ModuleReleaseAnnotationDeckhouseNodeName)
			moduleDir := path.Join(l.downloadedModulesDir, moduleName, downloader.DefaultDevVersion)
			if err := os.RemoveAll(moduleDir); err != nil {
				return fmt.Errorf("delete the '%s' stale directory of the '%s' module: %w", moduleDir, moduleName, err)
			}
			...
			if err := l.client.Update(ctx, &mpo); err != nil {
				l.log.Warnf("failed to annotate the '%s' module pull override: %v", moduleName, err)
			}
		}

		// if annotations is ok - we have to check that the file system is in sync
		moduleSymLink := filepath.Join(l.symlinksDir, fmt.Sprintf("%d-%s", moduleWeight, moduleName))
		if _, err := os.Stat(moduleSymLink); err != nil {
			// module symlink not found
			l.log.Infof("the '%s' module symlink is absent on file system, restore it", moduleName)
			if os.IsNotExist(err) {
				if err = l.createModuleSymlink(moduleName, moduleImageTag, source, moduleWeight); err != nil {
					return fmt.Errorf("create module symlink: %w", err)
				}
				// some other error
			} else {
				return fmt.Errorf("chekc the '%s' module symlink: %w", moduleName, err)
			}
			// check if module symlink leads to current version
		} else {
			dstDir, err := filepath.EvalSymlinks(moduleSymLink)
			...
			if filepath.Base(dstDir) != downloader.DefaultDevVersion {
				l.log.Infof("the '%s' module symlink is incorrect, restore it", moduleName)
				if err = l.createModuleSymlink(moduleName, moduleImageTag, source, moduleWeight); err != nil {
					return fmt.Errorf("create the '%s' module symlink: %w", moduleName, err)
				}
			}
		}

		// sync registry spec
		if err := utils.SyncModuleRegistrySpec(l.downloadedModulesDir, moduleName, downloader.DefaultDevVersion, source); err != nil {
			return fmt.Errorf("sync the '%s' module's registry settings with the '%s' module source: %w", moduleName, source.Name, err)
		}
		l.log.Infof("resynced the '%s' module's registry settings with the '%s' module source", moduleName, source.Name)
	}
