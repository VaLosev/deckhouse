...
func (k *kubeAPI) DeployRelease(ctx context.Context, release *v1alpha1.ModuleRelease) error {
	...
	if err = k.updateModuleReleaseDownloadStatistic(context.Background(), release, ds); err != nil {
	...
	relativeModulePath := path.Join("../", moduleName, "v"+release.Spec.Version.String())
	...
	if err = utils.ValidateModule(def, values, k.log); err != nil {
		...
		return fmt.Errorf("module '%s:v%s' validation: %w", moduleName, release.Spec.Version.String(), err)
	}
