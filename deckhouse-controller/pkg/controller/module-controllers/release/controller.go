...
func (r *reconciler) handleDeployedRelease(ctx context.Context, release *v1alpha1.ModuleRelease) (ctrl.Result, error) {
	...
	// check if RegistryAnnotationSpecChanged annotation is set process it
...
func (r *reconciler) handlePendingRelease(ctx context.Context, release *v1alpha1.ModuleRelease) (ctrl.Result, error) {
	...
	config, err := r.parseNotificationConfig(ctx)
	if err != nil {
		r.log.Errorf("failed to parse the '%s' notification config: %v", release.Name, err)
		return ctrl.Result{Requeue: true}, nil
	}
...
func (r *reconciler) deleteRelease(ctx context.Context, release *v1alpha1.ModuleRelease) (ctrl.Result, error) {
	...
	if err := os.RemoveAll(modulePath); err != nil {
		r.log.Errorf("failed to remove all in '%s': %v", modulePath, err)
		return ctrl.Result{Requeue: true}, nil
	}

	if release.Status.Phase == v1alpha1.ModuleReleasePhaseDeployed {
		extenders.DeleteConstraints(release.GetModuleName())
		symlinkPath := filepath.Join(r.downloadedModulesDir, "modules", fmt.Sprintf("%d-%s", release.Spec.Weight, release.Spec.ModuleName))
		if err := os.RemoveAll(symlinkPath); err != nil {
			r.log.Errorf("failed to remove all in '%s': %v", symlinkPath, err)
			return ctrl.Result{}, nil
		}
...
func (r *reconciler) cleanUpModuleReleases(ctx context.Context, release *v1alpha1.ModuleRelease) (ctrl.Result, error) {
	releases := new(v1alpha1.ModuleReleaseList)
...
func (r *reconciler) parseNotificationConfig(ctx context.Context) (updater.NotificationConfig, error) {
	...
	if err := r.client.Get(ctx, client.ObjectKey{Name: "deckhouse-discovery", Namespace: "d8-system"}, secret); err != nil {
